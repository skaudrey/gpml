...
%% interpolation
  ...
  out_index = 1;
 for typeD = 1:4 % space interpolation, space interpolation with smd feature, time interpolation, time inmterpoaltion with smd
  train_time = [];   inter_time = [];   rmse = {};  hyper = {}; lat_map = []; lon_map = [];
  out_index = 1;    
%   regmax = 0;
     if(typeD <= 2)
         stEnd = 20;batchsize = 1; 
     else
         stEnd = 40;batchsize = 1;      
     end
...
%         region = regionName(0,typeI,0,typeD,minlon,maxlon,minlat,maxlat);        
          filename = strcat(path,'Inter_vms_nospar_',spec_path(typeD),'.mat');    
...
%     end
 ...
 function [u0,u1,v0,v1,s0,s1,d0,d1,lat,lon,consum,inter,hyp] = vmsInter(typeS,typeI,typeD,typeM,path,minlon,minlat,maxlon,maxlat,epoch,samp,stamp)
%% global variables
...
switch(typeM)
    case 1 %pcareg_m_prod
        [u0 u1 x_lat x_lon  consum1 inter1 hypu] = time_mult_pca_sum_nospar(prefix_p,prefix_s,minlon,minlat,maxlon,maxlat,typeV(1),epoch,samp,stamp);
        [v0 v1 x_lat x_lon  consum2 inter2 hypv] = time_mult_pca_sum_nospar(prefix_p,prefix_s,minlon,minlat,maxlon,maxlat,typeV(2),epoch,samp,stamp);
