...
[hypu,hypv] = getMeanHyperOfVMS(1,1);    %get the mean hyper of phoenix,with method mult_pca_prod
...
basepath = cd;
normpath = [basepath '/moi_prac/reuse/normalday/vms/'];
...
%% interpolation
  ...
  for typeReg = 4:6
      ...
      for stIndex = 1:12   %set region
          u_temp = []; v_temp = []; su_temp = []; sv_temp = []; ysu_temp = []; ysv_temp = [];   xtest_temp = {};    ytest_temp = {};    xtrain_temp = {};   ytrain_temp = {};
    [minlon,maxlon,minlat,maxlat,stamp] = getStampRegofVMS(stIndex,typeReg);
        ...
        for epoch_index = 1:epoch_size
            ...
            for typeM = m0:m1   % see function regionName
                ...
                [u0 u1 v0 v1 s0 s1 d0 d1 consum inter ysut ysvt xtemp ytemp xtrmp ytrmp] = ...
                    reuseVmsInter_r(typeS,typeI,typeD,typeM,path,minlon,minlat,maxlon,maxlat,ep,samp,stamp,hypu,hypv);
                ...
                su_temp{index} = su1;
                sv_temp{index} = sv1; 
                ysu_temp{index-1} = ysut;
                ysv_temp{index-1} = ysvt;
...
%                 end
        ...
        rmse{regIndex} = rmse_temp;
        u{regIndex} = u_temp;
        v{regIndex} = v_temp;
        su{regIndex} = su_temp;
        sv{regIndex} = sv_temp;
        ysu{regIndex} = ysu_temp;         
        ysv{regIndex} = ysv_temp;
        x_test{regIndex} = xtest_temp;
        y_test{regIndex} = ytest_temp;
        x_train{regIndex} = xtrain_temp;
        y_train{regIndex} = ytrain_temp;
...
%         region = regionName(0,typeI,0,typeD,minlon,maxlon,minlat,maxlat);        
        filename = strcat(path,'Inter_vms_reuse_t_regionall_r','.mat');    
