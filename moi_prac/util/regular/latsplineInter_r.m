function [u0,u1,consum,inter,hyp] = twodsplineInter_r(file_pre,minlon,minlat,maxlon,maxlat,typeV,samp,stamp)
resolution = 0.125;
...
minLatInd = minlat/0.125+1;
maxLatInd = maxlat/0.125+1;

temp = linspace(1,size_lon,size_lon)';%distance matrix

xtrain = {}; xtest = {};ytrain = {};

temp_size = fix(size_lon/2);

x_train1 = temp(1:samp_inter:size_lon);
x_train2 = temp(samp_inter:samp_inter:size_lon);

index = 1;
for i = minLatInd:maxLatInd
    ytrain{index} = y_train(find(x_train(:,2) == i));
    if(mod(i,1)==0)
