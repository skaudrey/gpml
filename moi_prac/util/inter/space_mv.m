...
typeD = 1;  % 1 for lon; 2 for lat

typeV_t = 5;  % the variable type ,see in getPrefixByType, 1 for u10,2 for v10,3 for d, 4 for p, 5 for t
typeV_d = 3;
typeV_p = 4;
...
% for lat = minlat:resolution:maxlat
       prefix_t = getPrefixByType(typeS,typeI,typeV_t,typeD); % temperature
       prefix_p = getPrefixByType(typeS,typeI,typeV_p,typeD); % pressure
       prefix_d = getPrefixByType(typeS,typeI,typeV_d,typeD); % direction
       prefix_s = getPrefixByType(typeS,typeI,typeV,typeD); % u10
        
        [u0_temp u1_temp u2_temp] = mpV(prefix_t,prefix_p,prefix_d,prefix_s,lon);
        u0 = [u0,u0_temp];
        u1 = [u1,u1_temp];
        u2 = [u2,u2_temp];
end
...
% for lat = minlat:resolution:maxlat
       ...
       prefix_s = getPrefixByType(typeS,typeI,typeV,typeD); % v10
        ...
        [v0_temp v1_temp v2_temp] = mpV(prefix_t,prefix_p,prefix_d,prefix_s,lon);
        v0 = [v0,v0_temp];
        v1 = [v1,v1_temp];
        v2 = [v2,v2_temp];
end
...
for lon = minlon:resolution:maxlon
       ...
       prefix_s = getPrefixByType(typeS,typeI,typeV,typeD); % speed
        ...
        [s0_temp s1_temp s2_temp] = mpV(prefix_t,prefix_p,prefix_d,prefix_s,lon);
        s0 = [s0,s0_temp];
        s1 = [s1,s1_temp];
        s2 = [s2,s2_temp];
end
...
for lon = minlon:resolution:maxlon
        ...
        [d0_temp d1_temp d2_temp] = mp(file_prex,lon);
        d0 = [d0,d0_temp];
        d1 = [d1,d1_temp];
        d2 = [d2,d2_temp];
end
...
% region = strcat('t_multV_','LON',num2str(minlon),'-',num2str(maxlon));

typeM = 12; % 3 for multV,12 for mpv
