...
typeS = 3;  % EC dataset, and in regionname function this means 0.25 degree
...
%         [u0_temp u1_temp u2_temp] = multScale(file_prex,lon);
        [u0_temp u1_temp u2_temp] = mpng(file_prex,lat);
%         [u0_temp u1_temp u2_temp] = mp(file_prex,lat);       
%         [u0_temp u1_temp u2_temp] = mp(file_prex,lon);
        u0 = [u0,u0_temp];
        u1 = [u1,u1_temp];
        u2 = [u2,u2_temp];
end
...
%         [v0_temp v1_temp v2_temp] = multScale(file_prex,lon);
        [v0_temp v1_temp v2_temp] = mpng(file_prex,lat);
        v0 = [v0,v0_temp];
        v1 = [v1,v1_temp];
        v2 = [v2,v2_temp];
end
...
%         [d0_temp d1_temp d2_temp] = mp(file_prex,lat);
%         [d0_temp d1_temp d2_temp] = multScale(file_prex,lon);
        [d0_temp d1_temp d2_temp] = mpng(file_prex,lat);
        d0 = [d0,d0_temp];
        d1 = [d1,d1_temp];
        d2 = [d2,d2_temp];
end
...
%         [s0_temp s1_temp s2_temp] = multScale(file_prex,lon);
        [s0_temp s1_temp s2_temp] = mpng(file_prex,lat);
        s0 = [s0,s0_temp];
        s1 = [s1,s1_temp];
        s2 = [s2,s2_temp];
end
